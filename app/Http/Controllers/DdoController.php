...
class DdoController extends Controller
{
    ...
    public function applications(Request $request) {
    	...
    	return view('user.finance_executive.application-list', compact('applications', 'request'));
    }
    ...
    public function application_details($id) {
    	...
    	return view('user.finance_executive.application-details', compact('application', 'proposal', 'employee_documents', 'employee_nominees', 'statusHistory','add_recovery','service_form','organisation_details'));
    }
    ...
    public function applications_approval(Request $request) {
        try {
            ...
            if(!empty($form_field_master)) {
                $pension_application_form = DB::table('optcl_pension_application_form')->where('id', $request->application_id)->where('status', 1)->where('deleted', 0)->first();

                $application_form_field_status = DB::table('optcl_application_form_field_status')
                                ->select('id', 'status_id')
                                ->where('application_id', $request->application_id)
                                ->where('form_id', $form_field_master->form_id)
                ...
                if(empty($application_form_field_status)) {
                    ...
                    $form_field_status_history = [
                        'application_id' => $request->application_id,
                        'employee_id' => $pension_application_form->employee_id,
                        'employee_code' => $pension_application_form->employee_code,
                        'aadhaar_no' => $pension_application_form->employee_aadhaar_no,
                        'form_id' => $form_field_master->form_id,
                        'field_id' => $request->field_id,
                        'status_id' => $request->status_id,
                    ...
                    DB::table('optcl_application_form_field_status')
                        ->where('id', $application_form_field_status->id)
                        ->where('application_id', $request->application_id)
                        ->where('form_id', $form_field_master->form_id)
                        ->where('field_id', $request->field_id)
                        ->update($form_field_status);
                    ...
                    if(!empty($request->nominee_id)) {
                        DB::table('optcl_application_form_field_nominee_status_history')->insert([
                            'form_field_status_history_id' => $form_field_status_history_id,
                            'employee_nominee_id' => $request->nominee_id
                        ]);
                    }
                }
    ...
    public function applications_rejection(Request $request) {
        try {
            DB::beginTransaction();            
            $form_field_master = DB::table('optcl_pension_form_field_master')->where('id', $request->field_id)->first();
            if(!empty($form_field_master)) {
                ...
                $application_form_field_status = DB::table('optcl_application_form_field_status')
                                ->select('id', 'status_id')
                                ->where('application_id', $request->reject_application_id)
                                ->where('form_id', $form_field_master->form_id)
                                ->where('field_id', $request->field_id)
                                //->where('created_by', Auth::user()->id)
                                ->where('status', 1)
                ...
                if(empty($application_form_field_status)) {
                    //dd(1);
                    $form_field_status = [
                        'application_id' => $request->reject_application_id,
                        'form_id' => $form_field_master->form_id,
                        'field_id' => $request->field_id,
                        'status_id' => $request->status_id,
                        'remarks' => $request->remarks,
    ...
    public function get_rejection_remark(Request $request) {
        $field_id = $request->field_id;
        $fieldRemarks = DB::table('optcl_application_form_field_status')
                            ->where('field_id', $field_id)
                            ->where('created_by', Auth::user()->id)
                            ->first();
        if($fieldRemarks){
            echo $fieldRemarks->remarks;
        }else{
            echo "";
        }
    }
        ...
        try {
            ...
            if(!empty($application_form)) {
                ...
                if($request->application_status == 1) {
                    $status_updated_to = 10;                    
                    $message = "Your application has been approved by the Finance Executive. Please check the application details.";
                } else {
                    $status_updated_to = 11;
                    $message = "Your application has been returned by the Finance Executive. Please resubmit the required application details.";
                }
                ...
                DB::commit();
                Session::flash('success','Application has been submitted successfully!');
                return redirect()->route('ddo_applications');
            } else {
                DB::commit();
    ...
    /*public function applications_store_recovery(Request $request) {
        ...
        try {
            ...
            $application_status = ['application_status_id' => 18];
            ...
            // Notification Area
            $message = "Recovery details updated by the Finance Executive. Please check the application details.";
            Util::insert_notification($application->user_id, $application->id, $message);

            DB::commit();
            Session::flash('success','Recovery details updated successfully');
            return redirect()->route('ddo_applications');
        } catch (Exception $e) {
            DB::rollback();
        ...
        try {
            ...
            if(!empty($employee_recoveries)) {
                ...
                if(!empty($recovery_attachment_file)) {
                    ...
                    $recovery_attachment_path = $application->recovery_attachment;
                }
            }
    ...
    public function service_pension_form_submission(Request $request) {
        ...
        try {
            ...
            if(!empty($application_form)) {
                ...
                $service_pension_form['status_id'] = 19;
                ...
                DB::table('optcl_pension_application_form')->where('status', 1)->where('deleted', 0)->where('id', $request->application_id)->update(['application_status_id' => 19]);
                ...
                // Notification Area
                $message = "Part - II form updated by the Finance Executive. Please check the application details.";
                Util::insert_notification($application_form->user_id, $application_form->id, $message);
                // Notify Unit Head
                $message = "Part - II form updated by the Finance Executive with application no ".$application_form->application_no.". Please check the application details.";
                ...
                DB::commit();
                Session::flash('success','PART-II updated successfully');
                return redirect()->route('ddo_applications');
            } else {
                DB::commit();
