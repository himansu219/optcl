...
class FPDealingAssistantController extends Controller
{
    ...
    public function application_details($id) {
    	...
    	return view('user.dealing-assistant.fp-application-details', compact('application', 'proposal', 'employee_documents', 'employee_nominees', 'statusHistory', 'add_recovery', 'service_form', 'organisation_details'));
    }
    ...
    public function applications_submission(Request $request) {
        try {
            ...
            if(!empty($application_form)) {
                ...
                return redirect()->route('family_pension_da_applications');
            } else {
                DB::commit();
        ...
        try {
            ...
            if(!empty($employee_recoveries)) {
                foreach ($employee_recoveries as $key => $value) {
                    ...
                    $recovery_id = DB::table('optcl_nominee_add_recovery')->insertGetId($recovery);
                    ...
                    DB::table('optcl_nominee_add_recovery_history')->insert($recovery_history);
                }
            ...
            NomineeUtil::insert_notification($application->user_id, $application->id, $message);

            DB::commit();
            Session::flash('success','Recovery details added by Dealing Assistant successfully!');
            return redirect()->route('family_pension_application_details', $request->application_id);
        } catch (Exception $e) {
            DB::rollback();
            Session::flash('error','Something went wrong!');
            return redirect()->back();
    ...
    public function service_pension_form_submission(Request $request) {
        try {
            ...
            if(!empty($application_form)) {
                ...
                $service_pension_form_id = DB::table('optcl_nominee_pension_service_form')->insertGetId($service_pension_form);
                ...
                if(!empty($service_form)) {
                    foreach ($service_form as $key => $value) {
                        ...
                        $service_office_id = DB::table('optcl_nominee_pension_service_offices')->insertGetId($various_off);
                        //dd($various_off);
                ...
                DB::table('optcl_pension_application_form')
                    ...
                    ->where('deleted', 0)
                    ->where('id', $request->application_id)
                    ->update(['application_status_id' => 13]);
