...
class FPHRDealingAssistantController extends Controller
{
        ...
        try {
            ...
            if(!empty($application_form)) {
                ...
                if($request->application_status == 1) {
                    $to_update_status = 7;
                    $status = 'approved';
                    $message = "Application approved by HR Wing Dealing Assistant. Please check the application details.";
                    ...
                    $to_update_status = 8;
                    $status = 'returned';
                    $message = "Application return by HR Wing Dealing Assistant. Please resubmit the required field details.";
    ...
    public function fp_service_pension_form_submission(Request $request) {
        ...
        try {
            ...
            if(!empty($application_form)) {
                ...
                // Pensioner 
                NomineeUtil::insert_notification($appDetails->user_id, $appDetails->id, $message);
                $message = "Part - II form updated by HR Wing Dealing Assistant with application no ".$appDetails->application_no.". Please check the application details.";
                // Dealing Assistant
                $optcl_unit_id = DB::table('optcl_users')->where('designation_id', 1)->where('id', $appDetails->user_id)->value('optcl_unit_id');
                $n_user_id = DB::table('optcl_users')->where('designation_id', 2)->where('optcl_unit_id', $optcl_unit_id)->value('id');
                NomineeUtil::insert_notification($n_user_id, $appDetails->id, $message);
    ...
    public function sanction_order_submit(Request $request) {
        ...
        try {
            ...
            NomineeUtil::insert_notification($appDetails->user_id, $appDetails->id, $message);
            // Dealing Assistant
            $message = "Sanction order generated by HR Wing Sanctioning Authority with application no ".$appDetails->application_no.". Please check the application details.";
            $optcl_unit_id = DB::table('optcl_users')->where('designation_id', 1)->where('id', $appDetails->user_id)->value('optcl_unit_id');
            $n_user_id = DB::table('optcl_users')->where('designation_id', 2)->where('optcl_unit_id', $optcl_unit_id)->value('id');
            NomineeUtil::insert_notification($n_user_id, $appDetails->id, $message);
            // Finance Executive
