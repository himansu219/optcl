...
class HRDealingAssistantController extends Controller
{
    ...
    public function applications(Request $request) {
    	...
    	$applications = DB::table('optcl_pension_application_form as a')
                    ->select('a.*', 'pt.pension_type', 'c.status_name', 'a.recovery_attachment')
    	...
    	return view('user.hr-wing.dealing-assistant.application-list', compact('applications', 'request'));
    }
    ...
    public function fp_application_details($id) {
        ...
        if($is_commutation_pension_applied == 1){
    ...
    public function application_details($id) {
        ...
        return view('user.hr-wing.dealing-assistant.application-details', compact('application', 'proposal', 'employee_documents', 'employee_nominees', 'statusHistory', 'add_recovery', 'service_form', 'service_form_three', 'total_da_amount', 'service_pension_due', 'family_pension_data', 'dcr_gratuity_exist', 'dcr_gratuity_value', 'service_pension_due_exist', 'commutation_value', 'commutation_two_value', 'commutation_three_value'));
    }
    ...
    public function fp_applications_approval(Request $request) {
        ...
        try {
            ...
            if(!empty($application_form)) {
                ...
                if($request->application_status == 1) {
                    $to_update_status = 7;
                    $status = 'approved';
                    $message = "Application approved by HR Wing Dealing Assistant with application no ".$application_form->application_no.". Please check the application details.";
                    $message2 = "Application approved by HR Wing Dealing Assistant with application no ".$application_form->application_no.". Please check the application details.";
                } else {
                    $to_update_status = 8;
                    $status = 'returned';
                    $message = "Application return by HR Wing Dealing Assistant with application no ".$application_form->application_no.". Please resubmit the required field details.";
                    $message2 = "Application returned by HR Wing Dealing Assistant with application no ".$application_form->application_no.". Please check the application details.";
                }
    ...
    public function fp_applications_submission(Request $request) {
        try {
            DB::beginTransaction();
            $user = Auth::user();
            ...
            if(!empty($application_form)) {
                ...
                $optcl_unit_id = DB::table('optcl_users')->where('designation_id', 1)->where('id', $appDetails->user_id)->value('optcl_unit_id');
                $n_user_id = DB::table('optcl_users')->where('designation_id', 2)->where('optcl_unit_id', $optcl_unit_id)->value('id');
                NomineeUtil::insert_notification($n_user_id, $appDetails->id, $message2);
                // Finance Executive
                $n_user_id = DB::table('optcl_users')->where('designation_id', 3)->where('optcl_unit_id', $optcl_unit_id)->value('id');
                NomineeUtil::insert_notification($n_user_id, $appDetails->id, $message2);
                // Unit Head
                ...
                DB::commit();
                Session::flash('success','Application has been '. $status .' successfully!');
                return redirect()->route('family_pension_hr_dealing_assistant', $request->application_id);
            } else {
                DB::commit();
                Session::flash('error','Application not found');
                return redirect()->back();
    ...
    public function application_resubmission(Request $request) {
        try {
            ...
            Util::insert_notification($appDetails->user_id, $appDetails->id, $message);
            // Dealing Assistant
            $message = "Application resubmitted to HR Wing Sanctioning Authority with application no ".$application->application_no.". Please check the application details.";
        ...
        try {
            ...
            if(!empty($employee_recoveries)) {
                foreach ($employee_recoveries as $key => $value) {                    
                    ...
                    DB::table('optcl_employee_add_recovery')->where('id', $value['id_value'])->update($recovery);
                    $recovery['recovery_id'] = $value['id_value'];
                    DB::table('optcl_employee_add_recovery_history')->insert($recovery);
                }
            }
            $application_status = [
            ...
            Util::insert_notification($appDetails->user_id, $appDetails->id, $message); 
            // Dealing Assistant
            $message = "Recovery updated by HR Wing Dealing Assistant with application no ".$application->application_no.". Please check the application details.";
            $optcl_unit_id = DB::table('optcl_users')->where('designation_id', 1)->where('id', $appDetails->user_id)->value('optcl_unit_id');
            $n_user_id = DB::table('optcl_users')->where('designation_id', 2)->where('optcl_unit_id', $optcl_unit_id)->value('id');
            Util::insert_notification($n_user_id, $appDetails->id, $message); 
            // Finance Executive
    ...
    public function service_pension_form_submission(Request $request) {
        try {
            ...
            if(!empty($application_form)) {
                ...
                DB::table('optcl_employee_pension_service_form')->where($whereCond)->update($service_pension_form);
                // Service form history
                $service_pension_form['service_form_id'] = $request->service_form_id;
                $service_pension_form['status_id'] = 22;
                DB::table('optcl_employee_pension_service_form_history')->insert($service_pension_form);
                ...
                if(!empty($service_form)) {
                    foreach ($service_form as $key => $value) {
                        ...
                        DB::table('optcl_employee_pension_service_offices')->where($whereCond2)->update($various_off);
                        ...
                        DB::table('optcl_employee_pension_service_offices_history')->insert($office_history);
                    }
                }
                DB::table('optcl_pension_application_form')->where('status', 1)->where('deleted', 0)->where('id', $request->application_id)->update(['application_status_id' => 22]);
                ...
                Util::insert_notification($appDetails->user_id, $appDetails->id, $message);
                // Dealing Assistant
                $message = "Part - II form updated by HR Wing Dealing Assistant with application no ".$application_form->application_no.". Please check the application details.";
        ...
        try {
            ...
            if(!empty($application_form)) {
                ...
                if($form_service_period_duly == 0){
                    ...
                    $form_service_period_duly_from = !empty($request->form_service_period_duly_from) ? date('Y-m-d', strtotime($request->form_service_period_duly_from)) : NULL;
                    $form_service_period_duly_to = !empty($request->form_service_period_duly_to) ? date('Y-m-d', strtotime($request->form_service_period_duly_to)) : NULL;
                }
                $form_service_period_absence = !empty($request->form_service_period_absence) ? $request->form_service_period_absence : 0;
                if($form_service_period_absence == 1){
                    $form_service_period_absence_from = NULL;
                    ...
                    $form_service_period_absence_from = !empty($request->form_service_period_absence_from) ? date('Y-m-d', strtotime($request->form_service_period_absence_from)) : NULL;
                    $form_service_period_absence_to = !empty($request->form_service_period_absence_to) ? date('Y-m-d', strtotime($request->form_service_period_absence_to)) : NULL;
                }
                $form_is_checked = !empty($request->form_is_checked) ? $request->form_is_checked : 0;
                if($form_is_checked == 0){
                    $form_reason_no_demand_certificate = !empty($request->form_reason_no_demand_certificate) ? $request->form_reason_no_demand_certificate : NULL;
                ...
                DB::commit();
                Session::flash('success','Part II form submitted successfully!');
                return redirect()->route('family_pension_hr_dealing_assistant', $application_id);
            } else {
                DB::commit();
                Session::flash('error','Application not found');
        ...
        try {
            ...
            if(!empty($application_form)) {
                $service_pension_form = [
                    ...
                    'interruption_service_to' => !empty($request->interruption_service_to) ? date('Y-m-d', strtotime($request->interruption_service_to)) : NULL,
                    ...
                    'extraordinary_leave_from' => !empty($request->extraordinary_leave_from) ? date('Y-m-d', strtotime($request->extraordinary_leave_from)) : NULL,
                    'extraordinary_leave_to' => !empty($request->extraordinary_leave_to) ? date('Y-m-d', strtotime($request->extraordinary_leave_to)) : NULL,
                    ...
                    'period_of_suspension_from' => !empty($request->period_of_suspension_from) ? date('Y-m-d', strtotime($request->period_of_suspension_from)) : NULL,
                    'period_of_suspension_to' => !empty($request->period_of_suspension_to) ? date('Y-m-d', strtotime($request->period_of_suspension_to)) : NULL,
                    ...
                    'work_charged_service_from' => !empty($request->work_charged_service_from) ? date('Y-m-d', strtotime($request->work_charged_service_from)) : NULL,
                    'work_charged_service_to' => !empty($request->work_charged_service_to) ? date('Y-m-d', strtotime($request->work_charged_service_to)) : NULL,
                    ...
                    'boy_service_from' => !empty($request->boy_service_from) ? date('Y-m-d', strtotime($request->boy_service_from)) : NULL,
                    'boy_service_to' => !empty($request->boy_service_to) ? date('Y-m-d', strtotime($request->boy_service_to)) : NULL,
                    ...
                    'any_other_service_from' => !empty($request->any_other_service_from) ? date('Y-m-d', strtotime($request->any_other_service_from)) : NULL,
                    'any_other_service_to' => !empty($request->any_other_service_to) ? date('Y-m-d', strtotime($request->any_other_service_to)) : NULL,
                    ...
                    'addition_of_qualifying_service_from' => !empty($request->addition_of_qualifying_service_from) ? date('Y-m-d', strtotime($request->addition_of_qualifying_service_from)) : NULL,
                    'addition_of_qualifying_service_to' => !empty($request->addition_of_qualifying_service_to) ? date('Y-m-d', strtotime($request->addition_of_qualifying_service_to)) : NULL,
                    ...
                    'date_of_commencement_pension' => !empty($request->date_of_commencement_pension) ? date('Y-m-d', strtotime($request->date_of_commencement_pension)) : NULL,
                    'date_of_acknowlegement_commutation' => !empty($request->date_of_acknowlegement_commutation) ? date('Y-m-d', strtotime($request->date_of_acknowlegement_commutation)) : NULL,
                    ...
                    'life_time_arrear_from' => !empty($request->life_time_arrear_from) ? date('Y-m-d', strtotime($request->life_time_arrear_from)) : NULL,
                    'life_time_arrear_to' => !empty($request->life_time_arrear_to) ? date('Y-m-d', strtotime($request->life_time_arrear_to)) : NULL,
                ...
                $service_pension_form_id = DB::table('optcl_employee_pension_service_form_three')->insertGetId($service_pension_form);

                $service_pension_form['form_three_id'] = $service_pension_form_id;
                $service_pension_form['modified_at'] = $this->current_date;
                $service_pension_form['modified_by'] = $user->id;
                DB::table('optcl_employee_pension_service_form_three_history')->insert($service_pension_form);
                ...
                foreach ($employee_recoveries as $key => $value) {                    
                    ...
                    $recovery_id = DB::table('optcl_employee_pension_service_form_three_recovery')->insertGetId($recovery);
                    ...
                    DB::table('optcl_employee_pension_service_form_three_recovery_history')->insert($recovery_history);
                }

                DB::table('optcl_pension_application_form')->where('status', 1)->where('deleted', 0)->where('id', $request->application_id)->update(['application_status_id' => 23]);
                ...
                // Notification Area
                $message = "Part - III form added by HR Wing Dealing Assistant. Please check the application details.";
                $application_id = $application_form->id;
                $appDetails = DB::table('optcl_pension_application_form')->where('id', $application_id)->first();
                $pension_user_id = $appDetails->user_id;
                // Pensioner      
                Util::insert_notification($appDetails->user_id, $appDetails->id, $message);
                // Dealing Assistant
                $message = "Part - III form added by HR Wing Dealing Assistant with application no ".$application_form->application_no.". Please check the application details.";
                ...
                DB::commit();
                Session::flash('success', 'PART  - III details successfully added by Dealing Assistant');
                return redirect()->route('hr_dealing_applications');
            } else {
                DB::commit();
                Session::flash('error','Application not found');
    ...
    public function fp_service_pension_form_three_submission(Request $request) {
        ...
        try {
            ...
            if(!empty($application_form)) {
                ...
                $service_pension_form_id = DB::table('optcl_nominee_pension_service_form_three')->insertGetId($service_pension_form);
                ...
                foreach ($employee_recoveries as $key => $value) {
                    ...
                    $recovery_id = DB::table('optcl_nominee_pension_service_form_three_recovery')->insertGetId($recovery);
