...
class HRExecutiveController extends Controller
{
    ...
    public function applications(Request $request) {
    	...
    	return view('user.hr-wing.hr_executive.application-list', compact('applications', 'request'));
    }
        ...
        try {
            ...
            if(!empty($application_form)) {
                ...
                if($request->application_status == 1) {
                    ...
                    $message2 = "Application approved by HR Executive with application no ".$application_form->application_no.". Please check the application details.";
                } else {
                    $to_update_status = 30; 
                    $status = 'returned';
    ...
    public function applications_store_recovery(Request $request) {
        try {
            ...
            // Dealing Assistant
            $message = "Recovery updated by HR Executive with application no ".$application->application_no.". Please check the application details.";
            $optcl_unit_id = DB::table('optcl_users')->where('designation_id', 1)->where('id', $appDetails->user_id)->value('optcl_unit_id');
            $n_user_id = DB::table('optcl_users')->where('designation_id', 2)->where('optcl_unit_id', $optcl_unit_id)->value('id');
    ...
    public function service_pension_form_submission(Request $request) {
        ...
        try {
            ...
            if(!empty($application_form)) {
                ...
                Util::insert_notification($appDetails->user_id, $appDetails->id, $message);
                // Dealing Assistant
                $message = "Part - II form updated by HR Executive with application no ".$application_form->application_no.". Please check the application details.";
        ...
        try {
            ...
            if(!empty($application_form)) {
                ...
                foreach ($employee_recoveries as $key => $value) {
                    ...
                    DB::table('optcl_employee_pension_service_form_three_recovery')
                            ->where($where)
                            ->update($recovery);
                    $recovery_history = [
                ...
                // Notification Area  
                $message = "Part - III form updated by HR Executive. Please check the application details.";          
                $application_id = $application_form->id;
                $appDetails = DB::table('optcl_pension_application_form')->where('id', $application_id)->first();
                $pension_user_id = $appDetails->user_id;
                // Pensioner
                Util::insert_notification($appDetails->user_id, $appDetails->id, $message);
                // Dealing Assistant
                $message = "Part - III form updated by HR Executive with application no ".$appDetails->application_no.". Please check the application details.";
                $optcl_unit_id = DB::table('optcl_users')->where('designation_id', 1)->where('id', $appDetails->user_id)->value('optcl_unit_id');
                $n_user_id = DB::table('optcl_users')->where('designation_id', 2)->where('optcl_unit_id', $optcl_unit_id)->value('id');
                Util::insert_notification($n_user_id, $appDetails->id, $message);
                // Finance Executive
                ...
                DB::commit();
                Session::flash('success', 'PART  - III updated successfully');
                return redirect()->route('hr_executive_applications');
            } else {
                DB::commit();
                Session::flash('error','Application not found');
    ...
    public function sanction_order_generate($id) {
        ...
        return view('user.hr-wing.hr_executive.sanction-order-generate', compact('application', 'proposal', 'service_form_three'));
    ...
    public function sanction_order_submit(Request $request) {
        ...
        try {
            ...
            if($application->application_status_id == 35){
                ...
                $to_update_status_id = 36;
                $routevalue = 'hr_executive_application_details';
            }else{
                // DA(HR Wing) Generation
                $to_update_status_id = 25;
                $routevalue = 'hr_dealing_application_details';
            }
            ...
            DB::commit();
            return redirect()->route($routevalue, array($application_id));
    ...
    public function gratuity_sanction_order_generate($id) {
        ...
        return view('user.hr-wing.hr_executive.gratuity-sanction-order-generate', compact('application', 'proposal', 'service_form_three', 'recoveries'));
    }
