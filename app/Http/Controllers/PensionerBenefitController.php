...
class PensionerBenefitController extends Controller
{
    ...
    public function calculate_pensionar_benefits($id) {
        ...
        $service_form = DB::table('optcl_employee_pension_service_form')->where('application_id', $id)->first();
        ...
        $form_three = DB::table('optcl_employee_pension_service_form_three')->where('application_id', $id)->first();

        // Service Pension Due
        $last_basic_pay = !empty($form_three->emolument_last_basic_pay) ? $form_three->emolument_last_basic_pay : $proposal->basic_pay_amount_at_retirement;
    ...
    public function calculate_rules(Request $request) {
    	...
    	try {
    		...
    		$service_form = DB::table('optcl_employee_pension_service_form')->where('application_id', $application_id)->first();

            $form_three = DB::table('optcl_employee_pension_service_form_three')->where('application_id', $application_id)->first();
    		...
    		if($rule_id == 7) {
                ...
                $recovery_details = DB::table('optcl_employee_add_recovery')->where('status', 1)->where('deleted', 0)->where('application_id', $application_id)->get();
    ...
    public function calculate_service_pension_save(Request $request) {
    	...
    	try {
    		...
    		if(!empty($rule_id) && !empty($application_id)) {
	    		$transaction = DB::table('optcl_pension_calculation_transaction')->where('application_id', $application_id)
                    ->where('employee_id', $employee_id)->where('rule_id', $rule_id)
                    ->where('calculation_type_id', $calculation_type_id)
                    ->where('is_latest', 1)->first();
	    		
	    		if(empty($transaction)) {
                    $checkCondType = DB::table('optcl_pension_calculation_transaction')->where('application_id', $application_id)->where('employee_id', $employee_id)->where('calculation_type_id', $calculation_type_id)->where('is_latest', 1)->get();

                    if($checkCondType->count() > 0) {
                        DB::table('optcl_pension_calculation_transaction')
		    		...
		    		$transaction_id = DB::table('optcl_pension_calculation_transaction')->insertGetId($calculation_transactions);
		    		...
		    		DB::table('optcl_pension_calculation_transaction')->where('application_id', $application_id)
		    		->where('employee_id', $employee_id)->where('rule_id', $rule_id)
                    ->where('calculation_type_id', $calculation_type_id)
                    ->where('is_latest', 1)
                ...
                DB::table('optcl_pension_calculation_history')->insert($calculation_transaction_history);
	...
	public function calculate_dcr_gratuity_save(Request $request) {
    	...
    	try {
    		...
    		if(!empty($rule_id) && !empty($application_id)) {
	    		$transaction = DB::table('optcl_pension_calculation_transaction')->where('application_id', $application_id)->where('employee_id', $employee_id)->where('rule_id', $rule_id)->where('is_latest', 1)->where('status', 1)->where('deleted', 0)->first();
	    		...
	    		if(empty($transaction)) {
		    		...
		    		DB::table('optcl_pension_calculation_transaction')->where('application_id', $application_id)->where('employee_id', $employee_id)->where('rule_id', $rule_id)->where('is_latest', 1)->where('status', 1)->where('deleted', 0)->update($calculation_transactions);
			    }
    ...
    public function get_family_pension_details(Request $request){
        ...
        if($rule_id == 5){
            ...
            if(count($details) == 2) {
                ...
                $pensionValue = DB::table('optcl_pension_calculation_transaction')
                            ->select('optcl_pension_calculation_transaction.*','optcl_calculation_rule_master.calculation_type_id')
                            ->join('optcl_calculation_rule_master', 'optcl_calculation_rule_master.id','=','optcl_pension_calculation_transaction.rule_id')
                            ->where('optcl_pension_calculation_transaction.application_id', $application_id)
                            ->where('optcl_pension_calculation_transaction.employee_id', $employee_id)
                            ->where('optcl_pension_calculation_transaction.is_latest', 1)
    ...
    public function save_transaction_details(Request $request){
        try {
            ...
            if(count($transaction_details) == 6){
                ...
                $dataGet = DB::table('optcl_pension_calculation_transaction')->where($checkCond)->first();
                ...
                if($dataGet) {
                    ...
                    DB::table('optcl_pension_calculation_transaction')->where($cond_tran)->update($transaction_data);                    
                } else {
                     $checkCondType = [
                        "application_id"            => $application_id,
                        "employee_id"               => $employee_id,
                    ...
                    $condType = DB::table('optcl_pension_calculation_transaction')->where($checkCondType)->get();
                    
                    if($condType->count() > 0) {
                        DB::table('optcl_pension_calculation_transaction')->where($checkCondType)->update([
                            'is_latest' => 0
                        ]);
                    }
                    ...
                    $pension_transaction_id = DB::table('optcl_pension_calculation_transaction')->insertGetId($transaction_data);
                }
                ...
                DB::table('optcl_pension_calculation_history')->insertGetId($transaction_history_data);
            }
            DB::commit();
        ...
        try {
            ...
            if(!empty($application_form)) {
                ...
                if($application_form->application_status_id == 34){
                    ...
                    $url = url('hr-executive/applications-details/' . $application_id);
                }else{
                    // DA(HR Wing) Generation
                    $to_update_status_id = 24;
                    $url = url('hr-dealing/applications-details/' . $application_id);
                }
