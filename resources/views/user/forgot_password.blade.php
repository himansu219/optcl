    ...
    <input type="hidden" name="user_id" id="user_id" value="">
      ...
      <div class="form-group">
        ...
        <div class="input-group">
          ...
          {{-- <i class="fa fa-info-circle" data-toggle="tooltip" data-placement="right" id="info-title" title="Your password must be minimum 6 characters long."></i>  --}}
       </div>
  ...
  // forgot password form submission validation
   $("#forgot_password_form").validate({
       ...
       submitHandler: function(form, event) { 
        debugger;
          ...
          var password = $('#password').val();
           $.ajax({
              type:'POST',
              url:'{{ route("pensioner_verify_otp") }}',
              data: formData,
              dataType: 'JSON',
              processData: false,
              contentType: false,
    ...
    $('#resend_otp').on('click',function(e){
      ...
      var user_id = $('#user_id').val();
      $('.page-loader').addClass('d-flex');
      $.post('{{ route("pensioner_resend_otp") }}',{
        "_token": "{{ csrf_token() }}",
        "user_id":user_id,
      },function(response){
          $('.page-loader').removeClass('d-flex');
