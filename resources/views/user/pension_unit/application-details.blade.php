...
<div class="content-wrapper">
	<div class="row">
		<div class="col-12 grid-margin">
            ...
            <form id="application-form" method="post" action="{{ route('pension_unit_applications_submission') }}" autocomplete="off" enctype="multipart/form-data">
            @elseif($application->application_status_id == 2)
            <form id="application-recovery-form" method="post" action="{{ route('pension_unit_applications_store_recovery') }}" autocomplete="off" enctype="multipart/form-data">
            @elseif($application->application_status_id == 12)
            <form id="application-form-2" method="post" autocomplete="off" action="{{ route('pension_unit_service_pension_form_submission') }}" enctype="multipart/form-data">
            @else
            <form>
            @endif
                @csrf
...
<div class="modal fade" id="application_remark" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true" data-backdrop="false">
    <form action="{{ route('pension_unit_applications_submission') }}" method="post" id="application_return_remark" accept-charset="utf-8">
        @csrf
...
<script type="text/javascript">
    $(document).ready(function() {
        ...
        $(document).on('change', '.checkApprove', function() {
            ...
            if(field_id != '') {
                $.ajax({
                    url:'{{ route("pension_unit_applications_approval") }}',
                    type:'post',
    ...
    $(document).ready(function() {
        ...
        $(document).on('change', '.form_period_to', function() {
            ...
            if(period_from != '') {
                $('.page-loader').addClass('d-flex');
                $.ajax({
                    url:'{{ route("pension_unit_get_year_month_day") }}',
                    type:'post',
